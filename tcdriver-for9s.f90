...
!     ******************************************************************
!     *                         tc2dftpl                               *
!     *   terrain correction via 2 dimensional fast fourier transform  *
!     *    with either mass-prism topographic model                    *
!     *          or    mass-line topographic model.                    *
!     *                                                                *
!     *    programmer:   yecai li,                                     *
...
!
!     parameter
!     max_rowxclm: the mulplication of the maximum number of rows
!                                   by the maximum number of cloumns.
!                  this parameter has to be changed, if necessary,
!                                                   in each subroutine.
!     maxsubs:     the maximum size of the complex two-dimensional array
!                  after 100% zero-padding.

!     subroutines called
!     dtdz_2d2:     compute two convolutions simultaneously.
!     s2dk_txtytz:  compute the kernel function for the first-order
!                   gradients (tx, ty & tz) of the topogrational 
!                   gravitational potential.
!     tcmp0t:       compute the zero-order term, i.e., the attraction
!                   of the temple plate.
!     iftafb:       compute the fourier  transform of b;
!                   mutiply a with f{b} and 
!                   do the inverse fourier transform of axf{b}
...
!     *************************************************************
!     *                        dtdz_2d2                           *
!     *    computes the vertical components of the first order    *
!     *    gradients of the topographi! gravitational potential   *
!     *          by doing two convolutions simultaneously         *
!     *                                                           *
!     *    programmer:   yecai li,                                *
...
!     ************************************************************
!     *                     iftafb                               *
!     *    0:  with given fourier transform a                    *
!     *        and untransformed array b                         *
!     *    1:  computes the fourier transform of b, get fb       *
!     *    2:  multiply a with fb, get a_fb                      *
!     *    3:  compute the inverse fourier transform of afb,     *
!     *          i.e., ifafb                                     *
!     *                                                          *
!     *    programmer:   yecai li,                               *
!     *                  dept of geomatics engineering           *
...
!
!   purpose             - compute the fast fourier transform of
!                           a complex valued 1,2 or 3 dimensional
!                           array
!
!   usage               - call fft3d (a,ia1,ia2,n1,n2,n3,ijob,iwk,rwk,
!                           cwk)
!
!   arguments    a      - complex array. a may be a three
!                           dimensional array of dimension n1 by n2
!                           by n3, a two dimensional array of
...
!                           inverse fourier transform (depending on
!                           the value of input parameter ijob).
!                ia1    - first dimension of the array a exactly
!                           as specified in the dimension statement
!                           in the calling program. (input)
!                ia2    - second dimension of the array a exactly
!                           as specified in the dimension statement
!                           in the calling program. (input)
!                n1     - limits on the first, second, and third
!                n2         subscripts of the array a, respectively.
!                n3         (input)
!                ijob   - input option parameter.
!                           if ijob is positive, the fast fourier
!                             transform of a is to be calculated.
!                           if ijob is negative, the inverse
!                             fast fourier transform of a is to be
!                             calculated.
!                iwk    - integer work vector of length
!                           6*max(n1,n2,n3)+150.
!                rwk    - real work vector of length
!                           6*max(n1,n2,n3)+150.
!                cwk    - complex work vector of length
!                           max(n2,n3).
!
!   precision/hardware  - single and double/h32
!                       - single/h32,h48,h60
!
!   reqd. imsl routines - fftcc
!
...
!
!                  x(i+1,j+1,k+1)=triple sum of a(l+1,m+1,n+1)*
!                  exp(2*pi*sqrt(-1)*(i*l/n1+j*m/n2+k*n/n3))
!                  with l=0...n1-1, m=0...n2-1, n=0...n3-1
!                  and pi=3.1415...
!
!                if ijob is negative, fft3d calculates the inverse
!                fourier transform, x, according to the following
!                formula
!
!                  x(i+1,j+1,k+1)=1/(n1*n2*n3)*triple sum of
!                  a(l+1,m+1,n+1)*
!                  exp(-2*pi*sqrt(-1)*(i*l/n1+j*m/n2+k*n/n3))
!                  with l=0...n1-1, m=0...n2-1, n=0...n3-1
!                  and pi=3.1415...
...
!            2.  if a is a two dimensional array, set n3 = 1.
!                if a is a one dimensional array (vector),
!                set ia2 = n2 = n3 = 1.
!
